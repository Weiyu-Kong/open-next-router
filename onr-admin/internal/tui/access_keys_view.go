...
func newAccessKeysModel(service *adminService) accessKeysModel {
	...
	labels := []string{"name", "subject type", "subject id", "account id (optional)", "route policy id (optional)", "allowed providers (optional)", "allowed models (optional)", "expires at (RFC3339, optional)"}
	for i := range inputs {
...
func (m accessKeysModel) createCmd() tea.Cmd {
	...
	return func() tea.Msg {
		...
		if values[7] != "" {
			parsed, err := time.Parse(time.RFC3339, values[7])
		...
		secret, err := m.service.CreateAccessKey(context.Background(), values[0], values[1], values[2], values[3], values[4], values[5], values[6], expires, metadata)
...
func renderAccessDetail(record controlplane.AccessKeyRecord, state controlplane.SubjectState) string {
	...
	return fmt.Sprintf("Access key: %s\n\nstatus: %s\naccount: %s\nroute policy: %s\nproviders: %s\nmodels: %s\nsubject: %s/%s\nblocked: %s\ncreated: %s\nexpires: %s\nversion: %d\nmetadata: %s\n\nEsc back", record.Name, record.Status, accountID, valueOrDash(record.RoutePolicyID), listOrDash(record.AllowedProviders), listOrDash(record.AllowedModels), record.SubjectType, record.SubjectID, blocked, record.CreatedAt.Format(time.RFC3339), expires, record.Version, metadata)
}
