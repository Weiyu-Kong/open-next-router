...
const pageTitles={overview:"概览",access:"访问密钥",models:"模型目录",providers:"服务商配置",billing:"账单 / Redis",dumps:"请求日志",test:"测试控制台"};
const pageKickers={overview:"运维 / 概览",access:"控制面板 / 访问",models:"Meter / 模型",providers:"DSL 工作区 / 服务商",billing:"运行状态 / 账单",dumps:"请求取证 / 日志",test:"安全探测 / 测试"};
let overviewTimer=0;
let adminModelCatalog=[];
...
function showPage(page){document.querySelectorAll(".page").forEach(e=>e.classList.toggle("active",e.id==="page-"+page));document.querySelectorAll(".nav-item").forEach(e=>e.classList.toggle("active",e.dataset.page===page));document.getElementById("pageTitle").textContent=pageTitles[page];document.getElementById("pageKicker").textContent=pageKickers[page];if(page==="overview"||page==="billing")refreshOverview();if(page==="billing")refreshAdminMeter();if(page==="access")refreshKeys();if(page==="models")refreshAdminModels();if(page==="providers")refreshProviders().then(runEditorAnalysis).catch(e=>notify(e.message,true))}
...
function modelIsAvailable(model){return Object.values(model.providers||{}).some(mapping=>mapping?.supported)}
function providerMappingSummary(model){const entries=Object.entries(model.providers||{});if(!entries.length)return "尚未配置供应商映射";return entries.map(([provider,mapping])=>mapping?.supported?`${provider}: ${mapping.model_id||model.id}`:`${provider}: ${mapping?.note||"不支持"}`).join(" · ")}
async function refreshAdminModels(){if(!adminToken)return;const target=document.getElementById("adminModels");try{const d=await api("/api/admin/models"),models=d.models||[];adminModelCatalog=models;document.getElementById("supportedModelOptions").innerHTML=models.map(model=>`<option value="${escapeText(model.id)}"></option>`).join("");target.innerHTML=models.map(model=>{const p=model.pricing||{},available=modelIsAvailable(model),prices=[p.input&&`输入 ${p.input}`,p.output&&`输出 ${p.output}`,p.cache_hit&&`缓存 ${p.cache_hit}`].filter(Boolean).join(" · ");return `<article class="model-card"><div class="model-card-head"><h3>${escapeText(model.id)}</h3><span class="badge ${available?"":"revoked"}">${available?"可调用":"待接入"}</span></div><p>${escapeText(model.provider||"通用模型")}</p><strong>${escapeText(prices||"价格暂未配置")}</strong><small>${escapeText(p.unit||"")}</small><small class="model-mapping">${escapeText(providerMappingSummary(model))}</small></article>`}).join("")||'<div class="empty">暂无模型。</div>'}catch(e){target.innerHTML=`<div class="empty">${escapeText(e.message)}</div>`}}
function modelPickerHTML(id,selectedModels=[]){const selected=new Set((selectedModels||[]).map(String)),models=[...adminModelCatalog];for(const model of selected){if(!models.some(item=>item.id===model))models.push({id:model,provider:"已配置",providers:{}})}return `<fieldset class="model-picker-field"><legend>允许的模型 <span class="muted">不选择表示不限制</span></legend><div class="model-picker-toolbar"><input type="search" data-model-filter="${escapeText(id)}" placeholder="筛选模型"><span class="model-picker-count" id="${escapeText(id)}Count"></span><button type="button" class="ghost" data-model-action="all" data-model-picker="${escapeText(id)}">全选</button><button type="button" class="ghost" data-model-action="none" data-model-picker="${escapeText(id)}">清空</button></div><div class="model-picker" id="${escapeText(id)}">${models.map(model=>`<label class="model-picker-option" data-model-search="${escapeText(`${model.id} ${model.provider||""}`.toLowerCase())}"><input type="checkbox" value="${escapeText(model.id)}" ${selected.has(model.id)?"checked":""}><span><strong>${escapeText(model.id)}</strong><small>${escapeText(model.provider||"通用模型")} · ${modelIsAvailable(model)?"当前可调用":"目录模型"}</small></span></label>`).join("")}</div></fieldset>`}
function selectedModelValues(id){return [...document.querySelectorAll(`#${id} input[type="checkbox"]:checked`)].map(input=>input.value).join(",")}
function bindModelPicker(id){const picker=document.getElementById(id),filter=document.querySelector(`[data-model-filter="${id}"]`),count=document.getElementById(`${id}Count`);if(!picker)return;const update=()=>{const checked=picker.querySelectorAll('input[type="checkbox"]:checked').length;count.textContent=checked?`已选 ${checked}`:"不限模型"};picker.onchange=update;if(filter)filter.oninput=()=>{const query=filter.value.trim().toLowerCase();picker.querySelectorAll(".model-picker-option").forEach(option=>option.classList.toggle("hidden",!option.dataset.modelSearch.includes(query)))};document.querySelectorAll(`[data-model-picker="${id}"]`).forEach(button=>button.onclick=()=>{picker.querySelectorAll('input[type="checkbox"]').forEach(input=>input.checked=button.dataset.modelAction==="all");update()});update()}
...
document.getElementById("modelsRefreshBtn").onclick=refreshAdminModels;
...
let adminProviders=[];
let adminProviderPriority=[];
function providerCoverage(provider){return adminModelCatalog.filter(model=>model.providers?.[provider]?.supported).length}
function orderedProviders(){const seen=new Set(),out=[];for(const provider of [...adminProviderPriority,...adminProviders]){const name=String(provider||"").trim().toLowerCase();if(name&&!seen.has(name)){seen.add(name);out.push(name)}}return out}
function renderProviderPriority(){const target=document.getElementById("providerPriorityList"),providers=orderedProviders();if(!target)return;target.innerHTML=providers.map((provider,index)=>`<div class="provider-priority-item"><span class="provider-priority-rank">${index+1}</span><span class="provider-priority-copy"><strong>${escapeText(provider)}</strong><small>覆盖 ${providerCoverage(provider)} 个公开模型${index===0?" · 优先尝试":" · 不支持时继续回退"}</small></span><span class="provider-priority-actions"><button type="button" class="ghost" data-provider-move="up" data-provider-index="${index}" aria-label="上移 ${escapeText(provider)}" ${index===0?"disabled":""}>↑</button><button type="button" class="ghost" data-provider-move="down" data-provider-index="${index}" aria-label="下移 ${escapeText(provider)}" ${index===providers.length-1?"disabled":""}>↓</button></span></div>`).join("")||'<div class="empty">暂无供应商。</div>';target.querySelectorAll("[data-provider-move]").forEach(button=>button.onclick=()=>{const index=Number(button.dataset.providerIndex),next=button.dataset.providerMove==="up"?index-1:index+1;if(next<0||next>=adminProviderPriority.length)return;[adminProviderPriority[index],adminProviderPriority[next]]=[adminProviderPriority[next],adminProviderPriority[index]];renderProviderPriority()})}
refreshProviders=async function(){if(!adminToken)return;const [providersData,priorityData]=await Promise.all([api("/api/providers"),api("/api/admin/provider-priority")]);adminProviders=providersData.providers||[];adminProviderPriority=priorityData.providers||adminProviders;providerSelect.innerHTML="";const cards=document.getElementById("providerCards");if(cards)cards.innerHTML="";const count=document.getElementById("providerCount");if(count)count.textContent=`${adminProviders.length} 个`;for(const p of orderedProviders()){const opt=document.createElement("option");opt.value=p;opt.textContent=p;providerSelect.appendChild(opt);if(cards){const card=document.createElement("article");card.className="provider-card";card.innerHTML=`<div><span class="provider-status"></span><strong>${escapeText(p)}</strong></div><small>支持 ${providerCoverage(p)} 个公开模型</small><button class="ghost provider-card-action" type="button">管理配置</button>`;card.querySelector("button").onclick=()=>{providerInput.value=p;providerSelect.value=p;loadProvider()};cards.appendChild(card)}}if(adminProviders.length&&!providerInput.value)providerInput.value=orderedProviders()[0];renderProviderPriority()};
document.getElementById("saveProviderPriorityBtn").onclick=async()=>{try{const data=await api("/api/admin/provider-priority",{method:"PUT",headers:{"content-type":"application/json"},body:JSON.stringify({providers:adminProviderPriority})});adminProviderPriority=data.providers||adminProviderPriority;renderProviderPriority();notify(data.reload_required?"供应商顺序已保存。请重载 ONR 使运行时生效。":"供应商顺序已保存。") }catch(e){notify(e.message,true)}};
function providerPickerHTML(selected){const chosen=new Set((selected?.length?selected:orderedProviders()).map(String));return `<fieldset class="provider-picker"><legend>允许的供应商</legend><div class="provider-picker-grid">${orderedProviders().map((provider,index)=>`<label class="provider-picker-option"><input type="checkbox" value="${escapeText(provider)}" ${chosen.has(provider)?"checked":""}><span><strong>${escapeText(provider)}</strong><small>优先级 ${index+1} · 支持 ${providerCoverage(provider)} 个模型</small></span></label>`).join("")}</div></fieldset>`}
function selectedProviderValues(containerID){return [...document.querySelectorAll(`#${containerID} input[type="checkbox"]:checked`)].map(input=>input.value).join(",")}
newKey=function(){openModal(`<div class="eyebrow">访问密钥</div><h3>创建访问密钥</h3><div class="form-grid"><label>名称<input id="newName" autocomplete="off"></label><label>主体类型<input id="newType" value="api_key"></label><label>主体 ID<input id="newSubject" autocomplete="off"></label><label>账户 ID <span class="muted">可选，默认使用主体 ID</span><input id="newAccount" autocomplete="off"></label><label>路由策略 ID <span class="muted">可选</span><input id="newRoutePolicy" autocomplete="off"></label><label>过期时间 <span class="muted">可选，RFC3339 UTC</span><input id="newExpiry" placeholder="2030-01-01T00:00:00Z"></label><div id="newProviderPicker" class="provider-picker-wrap">${providerPickerHTML([])}</div></div><p class="unrestricted-note">默认已勾选全部供应商；系统按上方优先级选择支持请求模型的第一个供应商。API Key 池绑定会自动分配。</p><div class="modal-actions"><button id="createKeySubmit">创建密钥</button></div>`);document.getElementById("createKeySubmit").onclick=async()=>{try{const providers=selectedProviderValues("newProviderPicker");if(!providers)throw new Error("请至少选择一个供应商");const d=await api("/api/admin/access-keys",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({name:document.getElementById("newName").value.trim(),subject_type:document.getElementById("newType").value.trim(),subject_id:document.getElementById("newSubject").value.trim(),account_id:document.getElementById("newAccount").value.trim(),route_policy_id:document.getElementById("newRoutePolicy").value.trim(),allowed_providers:providers,allowed_models:"",provider_key_bindings:{},expires_at:document.getElementById("newExpiry").value.trim()})});openModal(`<div class="eyebrow">一次性密钥</div><h3>请立即保存此密钥。</h3>${d.pending?`<p class="error-text">配置等待处理：${escapeText(d.error||"请稍后从密钥管理中重试")}</p>`:""}<div class="secret">${escapeText(d.secret)}</div><button id="copySecretBtn">复制密钥</button>`);document.getElementById("copySecretBtn").onclick=()=>navigator.clipboard?.writeText(d.secret).then(()=>notify("密钥已复制。"));refreshKeys()}catch(e){notify(e.message,true)}}};
saveKeyRouting=async function(name,version,bindings){try{const providers=selectedProviderValues("editProviderPicker");if(!providers)throw new Error("请至少选择一个供应商");const allowed=new Set(providers.split(",")),keptBindings=Object.fromEntries(Object.entries(bindings||{}).filter(([provider])=>allowed.has(provider)));const body={allowed_providers:providers,allowed_models:"",provider_key_bindings:keptBindings,version};await api("/api/admin/access-keys/"+encodeURIComponent(name)+"/routing",{method:"PUT",headers:{"content-type":"application/json"},body:JSON.stringify(body)});notify("访问密钥路由已更新。");await refreshKeys();await manageKey(name)}catch(e){notify(e.message,true)}};
manageKey=async function(name){try{const d=await api("/api/admin/access-keys/"+encodeURIComponent(name)),k=d.record,pending=k.status==="pending";openModal(`<div class="eyebrow">访问密钥</div><h3>${escapeText(k.name)}</h3><p>${escapeText(k.subject_type)}/${escapeText(k.subject_id)} · ${escapeText(k.status)}</p><p class="muted">账户 ${escapeText(k.account_id||k.subject_id||"-")} · 路由策略 ${escapeText(k.route_policy_id||"-")}</p><div id="editProviderPicker">${providerPickerHTML(k.allowed_providers||[])}</div><p class="unrestricted-note">模型权限不受单个密钥限制；供应商将按全局优先级和模型可用性选择。</p><div class="button-row"><button id="saveRoutingBtn">保存路由</button></div>${pending?`<p class="error-text">${escapeText(k.provisioning_error||"配置等待处理。")}</p><div class="button-row"><button id="retryProvisionBtn">重试配置</button></div>`:`<div class="form-grid"><label>金额<input id="adjustAmount" inputmode="decimal" placeholder="10.00"></label><label>币种<input id="adjustCurrency" value="CNY"></label><label>幂等键<input id="adjustKey" placeholder="admin-2026-0001"></label></div><div class="button-row"><button id="creditKeyBtn">充值余额</button><button class="ghost" id="debitKeyBtn">扣除余额</button></div>`}<div class="button-row"><button id="rotateKeyBtn">轮换密钥</button><button class="ghost" id="revokeKeyBtn">撤销密钥</button></div>`);document.getElementById("saveRoutingBtn").onclick=()=>saveKeyRouting(name,Number(k.version||0),k.provider_key_bindings);if(pending){document.getElementById("retryProvisionBtn").onclick=()=>retryProvisioning(name)}else{document.getElementById("creditKeyBtn").onclick=()=>adjustBalance(name,"credit");document.getElementById("debitKeyBtn").onclick=()=>adjustBalance(name,"debit")};document.getElementById("rotateKeyBtn").onclick=()=>rotateKey(name);document.getElementById("revokeKeyBtn").onclick=()=>revokeKey(name)}catch(e){notify(e.message,true)}};
document.getElementById("newKeyBtn").onclick=newKey;

// Batch key administration: selection, provider routing, creation, revocation,
// and wallet adjustments all use one authenticated service endpoint.
function selectedKeyNames(){return [...document.querySelectorAll(".key-select:checked")].map(input=>input.value)}
async function batchAction(action,payload){const names=selectedKeyNames();if(!names.length){notify("请先选择至少一个访问密钥",true);return}try{const d=await api("/api/admin/access-keys/batch",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({action,names,...payload})}),results=d.results||[],failed=results.filter(item=>!item.ok),skipped=results.filter(item=>item.skipped);notify(failed.length?`完成，${failed.length} 个失败`:skipped.length?`批量操作完成，${skipped.length} 个余额已达阈值而跳过`:`批量操作完成` ,failed.length>0);await refreshKeys()}catch(e){notify(e.message,true)}}
function openBatchKeyModal(){openModal(`<div class="eyebrow">批量管理</div><h3>管理已选访问密钥</h3><p class="muted">当前选中 <strong id="batchSelectedCount">${selectedKeyNames().length}</strong> 个密钥。</p><div class="form-grid"><label>供应商路由<input id="batchProviders" placeholder="taotoken,ctyun"></label><label>额度金额<input id="batchAmount" inputmode="decimal" placeholder="10.00"></label><label>余额阈值 <span class="muted">可选，仅充值时生效</span><input id="batchThreshold" inputmode="decimal" placeholder="100.00"></label><label>币种<input id="batchCurrency" value="CNY"></label><label>幂等键前缀<input id="batchIdempotency" placeholder="batch-2026-09-10"></label></div><p class="muted">填写阈值后，只给当前余额严格低于阈值的已选密钥增加上述额度。</p><div class="button-row"><button id="batchCreateOpenBtn" class="ghost">批量创建</button><button id="batchRouteBtn">变更供应商</button><button id="batchCreditBtn">批量充值</button><button class="ghost" id="batchDebitBtn">批量扣款</button><button class="ghost" id="batchRevokeBtn">批量撤销</button></div>`);document.getElementById("batchCreateOpenBtn").onclick=batchCreateKeys;document.getElementById("batchRouteBtn").onclick=()=>batchAction("route",{allowed_providers:document.getElementById("batchProviders").value.trim()});document.getElementById("batchCreditBtn").onclick=()=>batchAction("credit",{amount:document.getElementById("batchAmount").value.trim(),threshold:document.getElementById("batchThreshold").value.trim(),currency:document.getElementById("batchCurrency").value.trim(),idempotency_prefix:document.getElementById("batchIdempotency").value.trim()});document.getElementById("batchDebitBtn").onclick=()=>batchAction("debit",{amount:document.getElementById("batchAmount").value.trim(),currency:document.getElementById("batchCurrency").value.trim(),idempotency_prefix:document.getElementById("batchIdempotency").value.trim()});document.getElementById("batchRevokeBtn").onclick=()=>{if(confirm("确认撤销选中的访问密钥？"))batchAction("revoke",{})}}
async function batchCreateKeys(){openModal(`<div class="eyebrow">批量创建</div><h3>创建一组访问密钥</h3><div class="form-grid"><label>名称前缀<input id="batchPrefix" value="batch-key"></label><label>数量<input id="batchCount" type="number" min="1" max="100" value="5"></label><label>供应商路由<input id="batchCreateProviders" placeholder="taotoken,ctyun"></label><label>主体类型<input id="batchSubjectType" value="api_key"></label></div><div class="modal-actions"><button id="batchCreateSubmit">创建并显示密钥</button></div>`);document.getElementById("batchCreateSubmit").onclick=async()=>{try{const d=await api("/api/admin/access-keys/batch",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({action:"create",prefix:document.getElementById("batchPrefix").value.trim(),count:Number(document.getElementById("batchCount").value),allowed_providers:document.getElementById("batchCreateProviders").value.trim(),subject_type:document.getElementById("batchSubjectType").value.trim()})});const rows=(d.results||[]).map(item=>`<tr><td>${escapeText(item.name)}</td><td class="secret">${escapeText(item.secret||item.error||"—")}</td></tr>`).join("");openModal(`<div class="eyebrow">批量创建结果</div><h3>请立即保存密钥</h3><div class="table-wrap"><table><thead><tr><th>名称</th><th>Secret / 错误</th></tr></thead><tbody>${rows}</tbody></table></div><div class="modal-actions"><button id="batchDoneBtn">完成</button></div>`);document.getElementById("batchDoneBtn").onclick=()=>{closeModal();refreshKeys()}}catch(e){notify(e.message,true)}}}
const originalRefreshKeys=refreshKeys;
refreshKeys=async function(){if(!adminToken)return;try{const d=await api("/api/admin/access-keys"),q=document.getElementById("keyFilter").value,rows=(d.records||[]).filter(k=>keyMatches(k,q));document.getElementById("keysBody").innerHTML=rows.map(k=>`<tr><td><input class="key-select" type="checkbox" value="${escapeText(k.name)}"></td><td><strong>${escapeText(k.name)}</strong></td><td><span class="badge ${escapeText(k.status)}">${escapeText(k.status)}</span></td><td>${escapeText(k.subject_type)}/${escapeText(k.subject_id)}</td><td>${k.created_at?new Date(k.created_at).toLocaleDateString():"—"}</td><td>${k.expires_at?new Date(k.expires_at).toLocaleDateString():"Never"}</td><td>${escapeText((k.allowed_providers||[]).join(", ")||"自动")}</td><td>${k.version||0}</td><td><button class="ghost key-action" data-name="${escapeText(k.name)}">管理</button></td></tr>`).join("");document.getElementById("keyEmpty").classList.toggle("hidden",rows.length!==0);document.querySelectorAll(".key-action").forEach(b=>b.onclick=()=>manageKey(b.dataset.name));const selectAll=document.getElementById("selectAllKeys");if(selectAll){selectAll.checked=false;selectAll.onchange=()=>document.querySelectorAll(".key-select").forEach(input=>input.checked=selectAll.checked)}}catch(e){notify(e.message,true)}};
document.getElementById("batchKeyBtn").onclick=openBatchKeyModal;
