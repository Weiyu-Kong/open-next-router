...
function keyMatches(k,q){q=q.toLowerCase();return !q||[k.name,k.status,k.subject_type,k.subject_id,k.account_id,k.route_policy_id].some(v=>String(v||"").toLowerCase().includes(q))}
...
function newKey(){openModal(`<div class="eyebrow">NEW CREDENTIAL</div><h3>Create access key</h3><div class="form-grid"><label>Name<input id="newName" autocomplete="off"></label><label>Subject type<input id="newType" value="api_key"></label><label>Subject ID<input id="newSubject" autocomplete="off"></label><label>Account ID <span class="muted">optional · defaults to subject ID</span><input id="newAccount" autocomplete="off"></label><label>Route policy ID <span class="muted">optional</span><input id="newRoutePolicy" autocomplete="off"></label><label>Expires at <span class="muted">optional · RFC3339 UTC</span><input id="newExpiry" placeholder="2030-01-01T00:00:00Z"></label></div><div class="modal-actions"><button id="createKeySubmit">Create key</button></div>`);document.getElementById("createKeySubmit").onclick=async()=>{try{const d=await api("/api/admin/access-keys",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({name:document.getElementById("newName").value.trim(),subject_type:document.getElementById("newType").value.trim(),subject_id:document.getElementById("newSubject").value.trim(),account_id:document.getElementById("newAccount").value.trim(),route_policy_id:document.getElementById("newRoutePolicy").value.trim(),expires_at:document.getElementById("newExpiry").value.trim()})});openModal(`<div class="eyebrow">ONE-TIME SECRET</div><h3>Save this secret now.</h3><div class="secret">${escapeText(d.secret)}</div><button id="copySecretBtn">Copy secret</button>`);document.getElementById("copySecretBtn").onclick=()=>navigator.clipboard?.writeText(d.secret).then(()=>notify("Secret copied."));refreshKeys()}catch(e){notify(e.message,true)}}}
