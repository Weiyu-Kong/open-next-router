...
var directiveMetadata = []DirectiveMetadata{
	...
	{Name: "set_path", Block: "upstream", Hover: "`set_path <expr>;`\n\nSets upstream request path. Expressions support `concat(...)`, `template(\"...\")`, the builtin `$request.*` / `$channel.*` / `$task.*` variables, and `if_present(\"<jsonpath>\", <then>, <else>)` which picks a branch by whether the client request body carries a value at the path (an explicit null, empty string, empty array or empty object counts as absent)."},
	...
	{Name: "filter_header_values", Block: "request", Hover: "`filter_header_values <header> <pattern>... [separator=\"<sep>\"];`\n\nRemoves upstream request header values that match any of the patterns; non-matching values are kept."},
	{Name: "keep_header_values", Block: "request", Hover: "`keep_header_values <header> <pattern>... [separator=\"<sep>\"];`\n\nKeeps only upstream request header values that match any of the patterns; non-matching values are removed."},
	...
	{Name: "json_filter_values", Block: "request", Hover: "`json_filter_values <jsonpath> <pattern>...;`\n\nRemoves values from a request JSON string array that match any of the patterns; non-matching values are kept."},
	{Name: "json_keep_values", Block: "request", Hover: "`json_keep_values <jsonpath> <pattern>...;`\n\nKeeps only values in a request JSON string array that match any of the patterns; non-matching values are removed."},
	...
	{Name: "req_inline_file", Block: "request", Hover: "`req_inline_file field=\"<multipart field>\" [max_bytes=<int>] [max_count=<int>];`\n\nReads a multipart file field into the request root so req_map builtins can reach the uploaded bytes; file parts are otherwise discarded. Each field lands under its own form field name as an array of {filename, content_type, b64} objects. Runs before validation and req_map. Files over max_bytes (default 20971520, max 67108864) or beyond max_count (default 4, max 16) are rejected, not truncated."},
	{Name: "req_map", Block: "request", Hover: "`req_map <mode>;`\n\nMap request JSON between API schemas.", Modes: []string{"openai_chat_to_openai_responses", "openai_chat_to_anthropic_messages", "openai_chat_to_gemini_generate_content", "openai_images_to_gemini_generate_content", "openai_images_edits_to_gemini_generate_content", "openai_images_to_minimax_image", "openai_images_to_qwen_image", "openai_videos_to_ppio", "anthropic_to_openai_chat", "gemini_to_openai_chat"}},
	...
	{Name: "json_filter_values", Block: "after_req_map", Hover: "`json_filter_values <jsonpath> <pattern>...;`\n\nRemoves values from a request JSON string array that match any of the patterns after req_map; non-matching values are kept."},
	{Name: "json_keep_values", Block: "after_req_map", Hover: "`json_keep_values <jsonpath> <pattern>...;`\n\nKeeps only values in a request JSON string array that match any of the patterns after req_map; non-matching values are removed."},
	...
	{Name: "resp_map", Block: "response", Hover: "`resp_map <mode>;`\n\nMap non-stream response JSON.", Modes: []string{"openai_responses_to_openai_chat", "anthropic_to_openai_chat", "gemini_to_openai_chat", "gemini_to_openai_images", "minimax_image_to_openai_images", "qwen_image_to_openai_images", "ppio_video_create_to_openai_videos", "ppio_video_result_to_openai_videos", "openai_to_anthropic_messages", "openai_to_gemini_chat", "openai_to_gemini_generate_content"}},
	...
	{Name: "resp_inline_url", Block: "response", Hover: "`resp_inline_url path=\"$.data[*].url\" set=<field> [when_request=\"$.response_format\" when_eq=<value>] [max_bytes=<int>] [timeout_ms=<int>] [concurrency=<int>];`\n\nNon-stream: fetches each URL at path and replaces it with its base64 content under `set`. Runs after `resp_map`, so paths address the downstream shape. This is the only response directive that performs network I/O: GET over http/https only, and every fetch is bounded by timeout_ms (default 30000, max 120000), max_bytes (default 10485760, max 67108864) and concurrency (default 4, max 16).\n\n`when_request`/`when_eq` gate the rule on a client request field, so a provider that normally returns links stays untouched. A failed or oversized fetch leaves the URL in place rather than failing the response."},
